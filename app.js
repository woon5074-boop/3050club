...
function seed() {
  ...
  names.forEach(function (n, i) { var l4 = String(1000 + i * 137).slice(-4); var id = n + '_' + l4; ids.push(id); members[id] = { name: n, last4: l4, age: ages[i % 3], region: '대구', status: 'approved', admin: i < 2, role: i === 0 ? '회장' : i === 1 ? '총무' : '정회원', joined: Date.now() - (400 - i * 20) * 86400000 }; priv[id] = { phone: '010-0000-' + l4 }; });
  members['신청자_9999'] = { name: '신청자', last4: '9999', age: '30대', region: '대구', status: 'pending', admin: false, role: '정회원', joined: Date.now(), ref: '김민준' }; priv['신청자_9999'] = { phone: '010-0000-9999' };
  ...
  var events = {
    e1: { title: (prev2.getMonth() + 1) + '월 정기월례회', date: iso(prev2), place: '체험용 파크골프장', fee: '20,000원', cap: 40, deadline: '', memo: '', scores: scores(1), hio: '' },
    e2: { title: (prev.getMonth() + 1) + '월 정기월례회', date: iso(prev), place: '체험용 파크골프장', fee: '20,000원', cap: 40, deadline: '', memo: '', scores: scores(2), hio: '박지훈 · A코스 7번 홀' },
    e3: { title: (next.getMonth() + 1) + '월 정기월례회', date: iso(next), place: '체험용 파크골프장 (A·B코스 36홀)', fee: '20,000원', cap: 40, deadline: '', memo: '집결 후 조 편성 발표, 라운드 종료 후 점심 식사가 있습니다.', scores: {}, hio: '' }
  };
  var rsvps = {}; ids.slice(1, 11).forEach(function (id) { rsvps['e3_' + id] = { eventId: 'e3', memberId: id, name: members[id].name, st: 'yes', at: Date.now() }; });
  var t = Date.now();
  var posts = {
    p1: { cat: '공지', authorId: ids[1], name: '이서연', body: '회원님들의 편리한 회비 납부를 위해 아래 계좌로 입금 부탁드립니다.\n국민은행 0304050777 · 예금주 3050파크골프', at: t - 5 * 86400000, likes: [ids[0], ids[2]], cc: 0, pin: true },
  ...
  return { members: members, members_private: priv, events: events, rsvps: rsvps, posts: posts, comments: comments, photos: photos, dues: dues, settings: { club: { fee: '20,000원', notice: '' } } };
}
...
function footer() { return '<div class="ft"><span class="b">' + CLUB.name + '</span><span>회비 계좌 ' + CLUB.bank + ' ' + CLUB.account + ' (예금주 ' + CLUB.holder + ')</span><span><a href="' + CLUB.band + '" target="_blank" rel="noopener">네이버 밴드 바로가기</a> · www.3050club.co.kr</span></div>'; }
function sheet(html) {
  closeSheet();
...
/* ---------- 마이 ---------- */
function vMy() {
  var h = header('마이페이지', '내 정보 · 회비 · 회원 명부');
...
function afterMy() {
  ...
  Promise.all([Store.list('dues'), Store.list('rsvps', { where: ['memberId', S.me.id] })]).then(function (r) {
    var dues = {}; r[0].forEach(function (d) { dues[d.id] = d.paid || {}; });
    ...
    box.innerHTML = '<section class="px" style="border-radius:24px;background:var(--g);color:#fff;padding:20px;display:flex;flex-direction:column;gap:16px"><div class="row" style="gap:14px">' + av(S.me.name, 60) + '<div class="grow"><div class="disp" style="font-size:24px">' + esc(S.me.name) + '</div><div class="row" style="gap:6px;margin-top:4px"><span class="chip w">' + esc(S.me.role || '정회원') + '</span>' + (S.me.age ? '<span class="chip dk">' + esc(S.me.age) + '</span>' : '') + '</div></div><button class="icb" style="background:transparent;border-color:#5C8C78;color:#fff" data-act="meEdit" aria-label="내 정보 수정">' + ic('pen', 18) + '</button></div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">' + [[myRank ? myRank + '위' : '-', '최근 순위'], [att + '회', '참석 신청'], [best ? best + '타' : '-', '베스트']].map(function (s) { return '<div style="background:var(--gd);border-radius:14px;padding:12px 0;text-align:center"><div class="disp" style="font-size:22px">' + s[0] + '</div><div style="font-size:11px;color:#CFE6D6">' + s[1] + '</div></div>'; }).join('') + '</div></section>' +
      '<section class="stack"><div class="sec"><h2>회비 납부 현황</h2></div><div class="px card" style="padding:16px;display:flex;flex-direction:column;gap:14px"><div style="display:grid;grid-template-columns:repeat(6,1fr);gap:6px">' + months.map(function (d) { var k = ym(d), fut = d > now, ok = (dues[k] || {})[S.me.id]; return '<div class="due ' + (ok ? 'ok' : fut ? '' : 'no') + '"><small>' + (d.getMonth() + 1) + '월</small>' + (ok ? '완납' : fut ? '예정' : '미납') + '</div>'; }).join('') + '</div><div class="row" style="border-radius:16px;background:var(--bg);padding:14px"><div class="grow"><div class="s12 mute">' + CLUB.bank + ' · 예금주 ' + CLUB.holder + '</div><div class="disp" style="font-size:22px;letter-spacing:1px">' + CLUB.account + '</div></div><button class="btn sm" data-act="copyAcc">' + ic('copy', 16) + '계좌 복사</button></div><p class="s12 mute">입금 후 운영진이 확인하면 납부 현황에 반영됩니다.' + (S.settings.fee ? ' 월회비 ' + esc(S.settings.fee) : '') + '</p></div></section>' +
...
function vAdmin() {
  ...
  return header('운영진 관리', '회원 승인 · 회비 · 월례회 운영', true) + '<main id="adBox" style="padding-top:18px"><div class="spin"></div></main>';
}
function afterAdmin() {
  if (!isAdmin()) return; var box = document.getElementById('adBox'), k = S.sub.dueM || ym();
  Promise.all([Store.list('members'), Store.get('dues', k), Store.list('members_private')]).then(function (r) {
    S.members = r[0]; var paid = (r[1] && r[1].paid) || {}, ph = {}; r[2].forEach(function (p) { ph[p.id] = p.phone; });
    var pend = S.members.filter(function (m) { return m.status === 'pending'; }), ok = S.members.filter(function (m) { return m.status === 'approved'; }).sort(function (a, b) { return a.name > b.name ? 1 : -1; }), unpaid = ok.filter(function (m) { return !paid[m.id]; }), ev = upcoming()[0];
    var ms = []; for (var i = 0; i < 6; i++) { var d = new Date(); d.setDate(1); d.setMonth(d.getMonth() - i); ms.push(ym(d)); }
    box.innerHTML = '<section class="px" style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px">' + [['전체 회원', ok.length + '명', 'var(--ink)'], ['가입 대기', pend.length + '명', pend.length ? 'var(--red)' : 'var(--ink)'], [k.slice(5) * 1 + '월 회비 미납', unpaid.length + '명', unpaid.length ? 'var(--red)' : 'var(--g)'], ['다음 월례회', ev ? dday(ev.date) : '-', 'var(--g)']].map(function (s) { return '<div class="card rv" style="padding:16px"><div class="s12 b mute">' + s[0] + '</div><div class="disp" style="font-size:28px;color:' + s[2] + '">' + s[1] + '</div></div>'; }).join('') + '</section>' +
      ...
      '<section class="stack"><div class="sec"><h2>회비 입금 확인</h2><label class="sr" for="dueM">월 선택</label><select id="dueM" class="inp" style="width:auto;min-height:44px">' + ms.map(function (m) { return '<option value="' + m + '"' + (m === k ? ' selected' : '') + '>' + m.replace('-', '년 ') + '월</option>'; }).join('') + '</select></div><div class="px card" style="padding:2px 16px">' + ok.map(function (m) { var p = !!paid[m.id]; return '<div class="row" style="padding:10px 0;border-bottom:1px solid var(--line)">' + av(m.name, 34) + '<div class="grow b" style="font-size:14px">' + esc(m.name) + '</div><span class="chip ' + (p ? '' : 'rose') + '">' + (p ? '완납' : '미납') + '</span><button class="btn ' + (p ? 'ghost' : '') + ' sm" data-act="dueToggle" data-id="' + esc(m.id) + '" data-k="' + k + '">' + (p ? '취소' : '입금 확인') + '</button></div>'; }).join('') + '</div></section>' +
      '<section class="stack"><div class="sec"><h2>월례회 운영</h2></div><div class="px" style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px">' + [['plus', '월례회 등록', '일정·장소·정원', 'evEdit', ''], ['shuffle', '조 자동 편성', '참석자 기준 4인 1조', 'teamAuto', ev ? ev.id : ''], ['pen', '스코어 입력', '최근 월례회 성적', 'scoreEdit', (past()[0] || ev || {}).id || ''], ['chat', '공지 올리기', '상단 고정 게시', 'postNew', '공지']].map(function (b) { return '<button class="card press" style="min-height:96px;padding:14px;display:flex;flex-direction:column;align-items:flex-start;justify-content:space-between;text-align:left" data-act="' + b[3] + '" data-id="' + b[4] + '" data-cat="' + b[4] + '"><span style="color:var(--g)">' + ic(b[0], 24) + '</span><span><span class="b" style="display:block">' + b[1] + '</span><span class="s12 mute">' + b[2] + '</span></span></button>'; }).join('') + '</div></section>' +
      '<section class="stack"><div class="sec"><h2>회원 관리</h2><button data-act="feeEdit">월회비 설정' + ic('arrow', 14) + '</button></div><div class="px card" style="padding:2px 16px">' + ok.map(function (m) { return '<button class="row" style="padding:10px 0;border:0;border-bottom:1px solid var(--line);background:none;width:100%;text-align:left" data-act="memEdit" data-id="' + esc(m.id) + '">' + av(m.name, 34) + '<span class="grow"><span class="b" style="font-size:14px;display:block">' + esc(m.name) + '</span><span class="s12 mute">' + esc(ph[m.id] || '') + '</span></span><span class="chip ' + (m.admin ? 'sun' : '') + '">' + esc(m.role || '정회원') + '</span></button>'; }).join('') + '</div></section>' + (DEMO ? '<div class="px"><button class="btn ghost block" data-act="demoReset">체험 데이터 초기화</button></div>' : '');
    reveal();
    document.getElementById('dueM').addEventListener('change', function (e) { S.sub.dueM = e.target.value; afterAdmin(); });
  });
}
...
var ACT = {
  ...
  dueToggle: function (d) { Store.get('dues', d.k).then(function (doc) { var paid = (doc && doc.paid) || {}; if (paid[d.id]) delete paid[d.id]; else paid[d.id] = true; return Store.set('dues', d.k, { paid: paid }); }).then(afterAdmin); },
  feeEdit: function () { sheet('<h3>월회비 설정</h3><form id="fFee" class="stack" style="gap:14px"><div class="field"><label for="fe">월회비 표시 금액</label><input id="fe" class="inp" value="' + esc(S.settings.fee || '') + '" placeholder="예: 20,000원"></div><button class="btn block">저장</button></form>'); document.getElementById('fFee').addEventListener('submit', function (e) { e.preventDefault(); Store.set('settings', 'club', { fee: val('fe') }, true).then(function () { S.settings.fee = val('fe'); closeSheet(); toast('저장되었습니다'); }); }); },
  memEdit: function (d) {
    var m = S.members.filter(function (x) { return x.id === d.id; })[0];
    sheet('<h3>' + esc(m.name) + ' 회원</h3><form id="fMem" class="stack" style="gap:14px"><div class="field"><label for="ro">직책</label><select id="ro" class="inp">' + ['회장', '부회장', '총무', '운영위원', '정회원', '신입'].map(function (r) { return '<option' + (m.role === r ? ' selected' : '') + '>' + r + '</option>'; }).join('') + '</select></div><label class="check"><input type="checkbox" id="ad"' + (m.admin ? ' checked' : '') + '><span><b>운영진 권한</b><br>회원 승인, 회비 확인, 월례회 등록·성적 입력이 가능합니다.</span></label><button class="btn block">저장</button>' + (m.id !== S.me.id ? '<button type="button" class="btn ghost block" style="color:var(--red)" data-act="memOut" data-id="' + esc(m.id) + '">탈퇴 처리</button>' : '') + '</form>');
    document.getElementById('fMem').addEventListener('submit', function (e) { e.preventDefault(); if (m.id === S.me.id && !document.getElementById('ad').checked) return toast('본인의 운영진 권한은 해제할 수 없습니다'); Store.set('members', m.id, { role: val('ro'), admin: document.getElementById('ad').checked }, true).then(function () { closeSheet(); toast('저장되었습니다'); afterAdmin(); }); });
  },
