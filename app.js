...
var DEFAULT_PARS = [4, 3, 4, 3, 5, 3, 4, 3, 4, 4, 3, 4, 3, 5, 3, 4, 3, 4];
function parsOf(ev) { var p = (ev.pars || '').split(',').map(function (x) { return parseInt(x, 10); }).filter(function (x) { return x >= 3 && x <= 5; }); return p.length === 18 ? p : DEFAULT_PARS; }
...
function afterMeet() {
  ...
  Promise.all([S.me ? Store.list('rsvps', { where: ['eventId', ev.id] }) : Promise.resolve([]), S.me ? Store.list('scorecards', { where: ['eventId', ev.id] }) : Promise.resolve([])]).then(function (rr) {
...
function openCard(ev, no) {
  var t = ev.teams[no - 1], c = (S._cards || {})[no] || { scores: {} }, pars = parsOf(ev), editable = !ev.done && (isAdmin() || (t.marker === S.me.id && !c.submitted));
  function cell(id, hidx) { var v = (c.scores[id] || [])[hidx]; return editable ? '<input type="number" inputmode="numeric" min="1" max="10" class="sc" data-p="' + esc(id) + '" data-h="' + hidx + '" value="' + (v || '') + '" aria-label="' + esc(mname(id)) + ' ' + (hidx + 1) + '번 홀">' : '<span class="sc ro" data-p="' + esc(id) + '" data-h="' + hidx + '">' + (v || '-') + '</span>'; }
  function half(from) { var rows = ''; for (var i = from; i < from + 9; i++) rows += '<tr><th>' + (i + 1) + '</th><td class="par">' + pars[i] + '</td>' + t.m.map(function (id) { return '<td>' + cell(id, i) + '</td>'; }).join('') + '</tr>'; return rows; }
  var head = '<tr><th>홀</th><th>PAR</th>' + t.m.map(function (id) { return '<th>' + esc(mname(id)) + '</th>'; }).join('') + '</tr>';
  sheet('<h3>' + no + '조 스코어카드</h3><p class="s13 mute">' + esc(ev.title) + ' · 조장 ' + esc(t.marker ? mname(t.marker) : '미지정') + (c.submitted ? ' · 제출 완료' : '') + '</p><div class="seg" id="scSeg"><button type="button" class="on" data-half="0">전반 1~9홀</button><button type="button" data-half="1">후반 10~18홀</button></div><div style="overflow-x:auto"><table class="sct" id="scT0"><thead>' + head + '</thead><tbody>' + half(0) + '</tbody></table><table class="sct" id="scT1" style="display:none"><thead>' + head + '</thead><tbody>' + half(9) + '</tbody></table></div><table class="sct"><tbody><tr><th style="width:64px">전반</th>' + t.m.map(function (id) { return '<td data-sum="f" data-p="' + esc(id) + '">-</td>'; }).join('') + '</tr><tr><th>후반</th>' + t.m.map(function (id) { return '<td data-sum="b" data-p="' + esc(id) + '">-</td>'; }).join('') + '</tr><tr><th>합계</th>' + t.m.map(function (id) { return '<td class="b" style="color:var(--g)" data-sum="t" data-p="' + esc(id) + '">-</td>'; }).join('') + '</tr></tbody></table><p class="s12 mute">노란색 홀인원 · 초록색 버디 이하. 홀별 타수는 더블파까지만 입력됩니다.</p>' + (editable ? '<div class="row"><button class="btn ghost grow" id="scSave">임시 저장</button><button class="btn grow" id="scSubmit">제출하기</button></div>' : (isAdmin() && c.submitted && !ev.done ? '<button class="btn ghost block" id="scReopen">제출 취소하고 다시 입력</button>' : '')));
  var root = document.getElementById('sheetbg');
  function read() { var sc = {}; t.m.forEach(function (id) { sc[id] = []; for (var i = 0; i < 18; i++) sc[id].push(0); }); root.querySelectorAll('.sc').forEach(function (el) { var v = Number(el.value !== undefined ? el.value : el.textContent) || 0, hx = +el.dataset.h; if (v > pars[hx] * 2) { v = pars[hx] * 2; if (el.value !== undefined) el.value = v; } sc[el.dataset.p][hx] = v; el.classList.toggle('hio', v === 1); el.classList.toggle('bd', v > 1 && v < pars[hx]); }); return sc; }
  function sums() { var sc = read(); t.m.forEach(function (id) { var f = 0, b = 0; sc[id].forEach(function (v, i) { if (i < 9) f += v; else b += v; }); root.querySelectorAll('[data-sum][data-p="' + (window.CSS && CSS.escape ? CSS.escape(id) : id) + '"]').forEach(function (td) { td.textContent = ({ f: f, b: b, t: f + b })[td.dataset.sum] || '-'; }); }); return sc; }
  sums(); root.addEventListener('input', sums);
  root.querySelectorAll('#scSeg button').forEach(function (b) { b.addEventListener('click', function () { root.querySelectorAll('#scSeg button').forEach(function (x) { x.classList.toggle('on', x === b); }); document.getElementById('scT0').style.display = b.dataset.half === '0' ? '' : 'none'; document.getElementById('scT1').style.display = b.dataset.half === '1' ? '' : 'none'; }); });
...
var ACT = {
  ...
  finalize: function (d) {
    ...
    Store.list('scorecards', { where: ['eventId', ev.id] }).then(function (cs) {
      ...
      var sc = {}, hio = [];
      sub.forEach(function (c) { Object.keys(c.scores || {}).forEach(function (id) { var hs = c.scores[id], f = 0, b = 0, full = true; hs.forEach(function (v, i) { if (!v) full = false; if (i < 9) f += v; else b += v; if (v === 1) hio.push(mname(id) + ' · ' + (i + 1) + '번 홀'); }); if (full) sc[id] = { f: f, b: b, h: hs }; }); });
